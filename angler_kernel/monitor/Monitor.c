...
asmlinkage int sys_LogicDroid_getRelationName(int sysID, char __user *relationName) {
  return LogicDroid_getRelationName(sysID, relationName);
}
...
int LogicDroid_initializeMonitor(int __user *UID, int appCount) {
  ...
  temp = copy_from_user(localUID, UID, sizeof(int) * localAppCount);
...
int LogicDroid_getRelationName(int sysID, char __user *relationName) {
  if (sysID < 0 || sysID >= relationSize)
    return OUT_OF_BOUND;

  if (notInitialized)
    return NO_MONITOR;

  strcpy(relationName, relations[sysID]);
