...
public class Monitor { 
  ...
  public boolean initializeMonitor()
  {
      ...
      for (ApplicationInfo ai : apps)
      {
        if (!mapping.containsKey(ai.uid))
        {
          ...
          appIdx++;
	  //Log.i("Monitor.java","Not contain: " + ai.processName + " " + ai.uid);
        }
        else
        {
          tempArr.get(mapping.get(ai.uid)).appendName(ai.processName);
	  //Log.i("Monitor.java","Contain: " + ai.processName + " " + ai.uid);
        }
      }
      app_num = tempArr.size();
      componentInfo = tempArr.toArray(new MonitorInfo[app_num]);

      for (MonitorInfo info : componentInfo)
      {
	//Log.i("Monitor.java","Name: " + info.name + " UID: " + info.UID);
  ...
  public boolean checkEvent(Event ev, long timestamp) {
    if (notInitialized) {
	//Log.i("Monitor.java: checkEvent","Not initalised");
      if (!initializeMonitor())
	//Log.i("Monitor.java: checkEvent","Not initalised, fail to initalised");
	...
	//Log.i("Monitor.java: checkEvent","ev.varcount false");
